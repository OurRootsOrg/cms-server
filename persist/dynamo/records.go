...
func (p Persister) SelectRecordHouseholdsForPost(ctx context.Context, postID uint32) ([]model.RecordHousehold, error) {
	qi := &dynamodb.QueryInput{
		...
		ExpressionAttributeValues: map[string]*dynamodb.AttributeValue{
			...
			":sk": {
				S: aws.String(recordHouseholdPrefix),
			},
		},
	}
	rhs := make([]model.RecordHousehold, 0)
	for {
		batch := make([]model.RecordHousehold, 0)
		qo, err := p.svc.Query(qi)
		if err != nil {
			log.Printf("[ERROR] Failed to get record households. qi: %#v err: %v", qi, err)
			return rhs, model.NewError(model.ErrOther, err.Error())
		}

		err = dynamodbattribute.UnmarshalListOfMaps(qo.Items, &rhs)
		if err != nil {
			log.Printf("[ERROR] Failed to unmarshal record households. qo: %#v err: %v", qo, err)
			return rhs, model.NewError(model.ErrOther, err.Error())
		}
		rhs = append(rhs, batch...)
		if qo.LastEvaluatedKey == nil {
			break
		}
		qi.ExclusiveStartKey = qo.LastEvaluatedKey
	...
	for i := range rhs {
		rhs[i].Household = strings.TrimPrefix(rhs[i].Type, recordHouseholdPrefix)
	}
	return rhs, nil
}
...
func (p Persister) SelectOneRecordHousehold(ctx context.Context, postID uint32, householdID string) (*model.RecordHousehold, error) {
	...
	gii := &dynamodb.GetItemInput{
		...
		Key: map[string]*dynamodb.AttributeValue{
			...
			skName: {
				S: aws.String(recordHouseholdPrefix + householdID),
			},
		},
	}
	gio, err := p.svc.GetItem(gii)
	if err != nil {
		log.Printf("[ERROR] Failed to get record household. qi: %#v err: %v", gio, err)
		return nil, model.NewError(model.ErrOther, err.Error())
	}
	if gio.Item == nil {
		return nil, model.NewError(model.ErrNotFound, strconv.Itoa(int(postID))+","+householdID)
	}
	err = dynamodbattribute.UnmarshalMap(gio.Item, &rh)
	if err != nil {
		log.Printf("[ERROR] Failed to unmarshal. qo: %#v err: %v", gio, err)
		return nil, model.NewError(model.ErrOther, err.Error())
	}
	rh.Household = strings.TrimPrefix(rh.Type, recordHouseholdPrefix)
	return &rh, nil
}
...
func (p Persister) InsertRecordHousehold(ctx context.Context, in model.RecordHouseholdIn) (*model.RecordHousehold, error) {
	var rh model.RecordHousehold
	var err error
	rh.RecordHouseholdIn = in
	rh.Type = recordHouseholdPrefix + in.Household
	now := time.Now().Truncate(0)
	rh.InsertTime = now
	rh.LastUpdateTime = now

	avs, err := dynamodbattribute.MarshalMap(rh)
	if err != nil {
		log.Printf("[ERROR] Failed to marshal record household %#v: %v", rh, err)
		return nil, model.NewError(model.ErrOther, err.Error())
	}
	// GSI sort key needs a value; we use PostID
	...
	twi[0] = &dynamodb.TransactWriteItem{
		ConditionCheck: &dynamodb.ConditionCheck{
			...
			Key: map[string]*dynamodb.AttributeValue{
				pkName: {
					S: aws.String(strconv.FormatInt(int64(rh.Post), 10)),
				},
				skName: {
					S: aws.String(postType),
				},
			...
			for i, r := range e.CancellationReasons {
				if r != nil && *r.Code != "None" {
					if *r.Code == "ConditionalCheckFailed" {
						...
						case 0:
							// This is the Post ID reference check
							return nil, model.NewError(model.ErrBadReference, strconv.FormatInt(int64(rh.Post), 10), postType)
						case 1:
							return nil, model.NewError(model.ErrOther, fmt.Sprintf("Insert failed. Record Household ID %s already exists", rh.Household))
						default: // i >= 2
							// Should never happen
							log.Printf("[ERROR] Failed to put record household %#v. twii: %#v err: %v", rh, twii, err)
							return nil, model.NewError(model.ErrOther, err.Error())
						}
					} else if *r.Code == "TransactionConflict" {
						log.Printf("[ERROR] TransactionConflict when putting record household %#v. twii: %#v err: %v", rh, twii, err)
						return nil, model.NewError(model.ErrConflict)
					} else {
						log.Printf("[ERROR] Failed to put record household %#v. twii: %#v err: %v", rh, twii, err)
						return nil, model.NewError(model.ErrOther, err.Error())
...
func (p Persister) DeleteRecordHouseholdsForPost(ctx context.Context, postID uint32) error {
	rhs, err := p.SelectRecordHouseholdsForPost(ctx, postID)
	if err != nil {
		return err
	}
	batchSize := 25
	...
	for _, r := range rhs {
		ris[*p.tableName] = append(ris[*p.tableName],
			&dynamodb.WriteRequest{
				DeleteRequest: &dynamodb.DeleteRequest{
					Key: map[string]*dynamodb.AttributeValue{
						pkName: {S: aws.String(strconv.FormatInt(int64(r.Post), 10))},
						skName: {S: aws.String(r.Type)},
					},
				},
			},
		)
		// log.Printf("[DEBUG] DeleteRecordHouseholdsForPost ris[%s][%d]: %#v", *p.tableName, i%batchSize, ris[*p.tableName][i%batchSize])

		if len(ris[*p.tableName]) == batchSize {
			log.Printf("[DEBUG] DeleteRecordHouseholdsForPost, batch ris: %#v", ris)
			// Delete a batch
			err := p.deleteRecordBatch(ris)
			if err != nil {
				return model.NewError(model.ErrOther, err.Error())
	...
	if len(ris[*p.tableName]) > 0 {
		log.Printf("[DEBUG] DeleteRecordHouseholdsForPost, final ris: %#v", ris)
		// Delete final batch
		err := p.deleteRecordBatch(ris)
		if err != nil {
			return model.NewError(model.ErrOther, err.Error())
