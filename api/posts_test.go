...
func doPostsTests(t *testing.T,
	...
	assert.Equal(t, model.ErrBadReference, err.(*model.Errors).Errs()[0].Code, "err.(*model.Errors).Errs()[0]: %#v", err.(*model.Errors).Errs()[0])
	...
	_, err = testApi.AddPost(context.TODO(), in)
	assert.IsType(t, &model.Errors{}, err)
	if assert.Len(t, err.(*model.Errors).Errs(), 1, "err.(*model.Errors).Errs(): %#v", err.(*model.Errors).Errs()) {
		assert.Equal(t, err.(*model.Errors).Errs()[0].Code, model.ErrRequired)
	}
