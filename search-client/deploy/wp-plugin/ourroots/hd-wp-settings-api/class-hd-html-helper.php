...
if ( ! class_exists( 'OURROOTS_HTML_Helper' ) ) :
/**
 * HTML Helper Class
 *
...
class OURROOTS_HTML_Helper {
	...
	public function get_field( $field, $show_help = true ) {
		...
		switch ( $field['type'] ) {
			case 'password'     : $input_html .= $this->password_input( $field ); break;
			case 'number'     : $input_html .= $this->number_input( $field ); break;
	...
	private function number_input( $field ) {
		...
		return sprintf(
			'<input type="number" name="%s" id="%s" value="%s" min="%s" max="%s" class="regular-text"/>',
			esc_attr( $field['id'] ),
			esc_attr( $field['id'] ),
			esc_attr( $field['value'] ),
			esc_attr( $field['min'] ),
	 ...
	 * @return null
	 */
	private function password_input( $field ) {
